...
<div class="w ofc case">
	...
	<div class="box bg_white pb">
		<dl class="distr">
			...
			<dd id="paymentCash"  style="display: none;">
				<label>付款方式：</label>
				<input type="radio" name="paymentCash" value="1" checked="checked"/>现金&nbsp;&nbsp;<input type="radio" name="paymentCash" value="2"/>POS刷卡
			</dd>
	...
	<div class="box bg_white pb">
		...
		<tbody>
		<c:forEach items="${userCart.items}" var="userItem">
			<tr>
				<td>${userItem.sku.product.id}</td>
				<td class="img48x20">
					<span class="inb"><img src="${userItem.sku.product.img.imgUrl}"></span>
					<a target="block" href="javascript:void(0);">${userItem.sku.product.name}--${userItem.sku.color.name}--${userItem.sku.size}</a>
				</td>
				<td>￥${userItem.sku.price}元</td>
				<td>x${userItem.amount}</td>
				<td>${userItem.sku.stock}</td>
			</tr>
		</c:forEach>
		</tbody>
		</table>
	 ...
	 <div class="confirm mt">
		...
		<div class="ofc">
			<div class="r">
				<dl class="total">
					<dt>订单金额：<cite>(共<var id="totalNum">${userCart.allProductAmount}</var>个商品)</cite></dt>
					<dd><em class="l">商品金额：</em>￥<var>${userCart.price}</var></dd>
					<dd><em class="l">返现：</em>￥<var>0.00</var></dd>
					<dd><em class="l">运费：</em>￥<var>${userCart.extra}</var></dd>
					<dd class="orange"><em class="l">应付总额：</em>￥<var id="totalMoney">${userCart.allPrice}</var></dd>
