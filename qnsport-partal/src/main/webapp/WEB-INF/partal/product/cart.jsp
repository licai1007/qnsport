...
function addUserItemAmount(skuId,upperLimit){
	...
	//异步提交
	var url = "/addUserItemAmount.html";
	var params = {
			"skuId":skuId,
			"amount":1
	};
...
//减少商品购买数量
function subUserItemAmount(skuId){
	var num = $('#num'+skuId).val();
	num--;
	if(num == 0){
		alert('至少购买一件！');
...
<div class="w ofc case">
	<div class="confirm">
		...
		<div class="ofc pb40">
			...
			<tbody>
			<c:forEach items="${userCart.items}" var="userSku">
				<tr>
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a class="pic" title="#" href="#"><img alt="#" src="${userSku.sku.product.img.imgUrl}"></a>
								<dl>
									<dt><a title="#" href="#"> ${userSku.sku.product.name }--${userSku.sku.color.name }--${userSku.sku.size }</a></dt>
									<dd><span class="red">【赠品】</span>
										<p class="box_d bg_gray2 gray"><a title="水杯" href="#">水杯</a><br></p>
									</dd>
								</dl> 
							</li>
						</ul>
					...
					<td>
						<a onclick="subUserItemAmount('${userSku.sku.id}')" class="inb arr" title="减" href="javascript:void(0);">-</a>
						<input type="text" id="num${userSku.sku.id}" readonly="readonly" value="${userSku.amount}" name="" size="1" class="txts">
						<a onclick="addUserItemAmount('${userSku.sku.id}','${userSku.sku.upperLimit}')" class="inb arr" title="加" href="javascript:void(0);">+</a>
						<dl style="margin-left: 20px;color: red">
						<c:if test="${userSku.isHave }">有货</c:if> 
						<c:if test="${!userSku.isHave }">无货</c:if> 
						</dl>
					</td>
			...
			<div class="page">
				<span class="l">
					<input type="button" onclick="window.location.href='http://localhost:8081';" class="hand btn100x26c" title="继续购物" value="继续购物">
